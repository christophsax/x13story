...
abstract: >
  ...
  adjustment. Thanks to the R packages *seasonal* and *x13binary*, installing X
  -13ARIMA-SEATS is as easy as installing any other R package. This provides a
  simple way of discussing and exchanging seasonal adjustment problems.
  Additionally, our new *x13story* package allows to document and publish
  discussions both as a traditional documentation or as an interactive online
  *story*.
...
Here is an example on how to use `dput` on the built in examples series
`AirPassengers`:
...
Gist](https://gist.github.com), or any other code sharing platform
...
production of pretty PDFs easy. These functions start with `pretty_`:
`prettyplot`, prettymonthplot` or `prettysummary`. A more extensive description
...
To initialize a view, use the `x13view` function in an R chunk:
...
For a list of all available `series` options, see the help page of the `series`
function in  the *seasonal* package. Basically, all possible output series from
X-13 are available. The text after an R chunk, written in Markdown, is rendered
to HTML and shown on the website. To see the available *stories* on the website,
click the graduation cap on the top left.

The *x13story* template described above has several examples on how to use the
`x13view` function. 

If you want to make your story publicly available, you can contribute it to the
`x13story` package. To do so, you need to fork the
...
appears on the website. (If that appears too complicated, send your file to one
of the authors. It is planned to add an offline version of the interactive tool
to the *x13story* package, so that the look and feel can be tested locally.)
