...
plot_dygraph <- function(m, series = "main"){
  if (series %in% c("main", "mainpc")){
    dta <- cbind(original = original(m), adjusted = final(m))
    if (series == "mainpc") dta <- PC(dta)
    d <- dygraph(dta)
    ...
    ser <- series(m, series, reeval = FALSE)
    if(!inherits(ser, "ts")){
  ...
  # d <- dyRangeSelector(d, height = 28, fillColor = "#16556f", strokeColor = "")

  d <- dyLegend(d, show = "always", width = 300, labelsDiv = "oLabel")
  d <- dyOptions(d, gridLineColor = "#E1E5EA", axisLineColor = "#303030")
  d
